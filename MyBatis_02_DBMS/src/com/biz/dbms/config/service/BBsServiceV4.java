...
public class BBsServiceV4 {
		...
		try {
			...
			if (yesNo.equals("YES")) {
				...
				if(strName.trim().length()>0) {
					dto.setBs_writer(strName.trim());
				}
				System.out.printf("제목 : ",dto.getBs_subject());
				String strSub = scan.nextLine();
				if(strSub.trim().length()>0) {
					dto.setBs_subject(strSub.trim());
				}
	...
	public void deleteBBs() {
		System.out.println("======================================================");
		System.out.print("삭제할 ID(종료 Q) : ");
		String strID = scan.nextLine();
		try {
			int intID = Integer.valueOf(strID);
			this.viewText(intID);
			System.out.println("------------------------------------------------------");
			System.out.print("삭제 YES/NO : ");
			String yesNo = scan.nextLine();
			if (yesNo.equals("YES")) {
				BBsDao dao = sqlSession.getMapper(BBsDao.class);
				dao.delete(intID);
